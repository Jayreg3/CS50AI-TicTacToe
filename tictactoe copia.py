...
def minimax(board):
    ...
    if terminal(board) is True:
        return None
    best_action = {'action': False, 'value': False, 'num_of_turns': float('inf')}

    if player(board) == 'X':
        for action in actions(board):
            action_value = max_value(result(board, action), 1)
            if (action_value['value'] >= best_action['value'] and action_value['num_of_turns'] <= best_action['num_of_turns']) or best_action['value'] is None:
                best_action['action'] = action
                best_action['value'] = action_value['value']
                best_action['num_of_turns'] = action_value['num_of_turns']
            print(best_action)
    else:
        for action in actions(board):
...
def max_value(board, num_of_turns):
    print('running max_value with board = ', board, ' and num_of_turns = ', num_of_turns)
    if terminal(board):
        return {'value': utility(board), 'num_of_turns': num_of_turns}
    value = float('-inf')
    for action in actions(board):
        value = max(value, min_value(result(board, action), num_of_turns + 1)['value'])
    return {'value': value, 'num_of_turns': num_of_turns}


def min_value(board, num_of_turns):
    print('running min_value with board = ', board, ' and num_of_turns = ', num_of_turns)
    if terminal(board):
        return {'value': utility(board), 'num_of_turns': num_of_turns}
    value = float('inf')
    for action in actions(board):
        value = min(value, max_value(result(board, action), num_of_turns + 1)['value'])
    return {'value': value, 'num_of_turns': num_of_turns}